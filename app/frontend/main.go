// Code generated by hertz generator.package mainimport (
	"context"
	"os"	"frontend/biz/router"
	"frontend/conf"
	"frontend/infra/rpc"
	"frontend/middleware"
	...
	_ = godotenv.Load()
	rpc.InitClient()	address := conf.GetConf().Hertz.Address	p := hertzotelprovider.NewOpenTelemetryProvider(
		hertzotelprovider.WithEnableMetrics(false),
	)
	defer p.Shutdown(context.Background())	h := server.New(server.WithHostPorts(address))
	h.LoadHTMLGlob("template/*")
	h.Delims("{{", "}}")
	registerMiddleware(h)	router.GeneratedRegister(h)	h.GET("sign-in", func(ctx context.Context, c *app.RequestContext) {
